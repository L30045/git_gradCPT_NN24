...
#%% for the first n_fig5_components of ref_subject's components, plot (1) the mean HRF
# +/- SD across ref_subject's own HRF and its temporal-matched HRF from every other
# subject, (2) a bar plot of each other subject's matched component index, and (3) a bar
# plot of each other subject's matched correlation -- one column per component, combined
# into one (enlarged, for readability) figure. A matched HRF with negative correlation
# to ref_subject's HRF is sign-flipped before averaging/plotting (it is the same shape
# inverted), and the correlation bar shows abs(corr) to match.
n_fig5_components = min(5, n_ref_components)
fig5, axs5 = plt.subplots(3, n_fig5_components, figsize=(6 * n_fig5_components, 14), sharex='row')
for ax in axs5[1, 1:]:
    ax.sharey(axs5[1, 0])
for ax in axs5[2, 1:]:
    ax.sharey(axs5[2, 0])

for ref_comp in range(n_fig5_components):
    ...
    for curve in group_hrfs:
        ax_hrf.plot(t_delay, curve, color='gray', alpha=0.3, linewidth=1.2)
    ax_hrf.plot(t_delay, mean_hrf, color='r', linewidth=3, label=f'mean (n={len(group_hrfs)})')
    ...
    ax_hrf.set_title(f'{ref_subject} #{ref_comp} ({ref_var_explained*100:.1f}%)', fontsize=16)
    ax_hrf.tick_params(axis='both', labelsize=12)
    ax_hrf.legend(fontsize=12)
    ax_hrf.grid()
    ...
    ax_comp.tick_params(axis='x', labelrotation=90, labelsize=12)
    ...
    ax_corr.tick_params(axis='x', labelrotation=90, labelsize=12)
    ax_corr.tick_params(axis='y', labelsize=12)
    ax_corr.grid(axis='y')

    if ref_comp == 0:
        ax_hrf.set_ylabel(f'{decomp_method.upper()} component (a.u.)', fontsize=13)
        ax_comp.set_ylabel('Matched component #', fontsize=13)
        ax_corr.set_ylabel('|Matched correlation|', fontsize=13)

fig5.supxlabel('Delay (s) (top row) / Subject (bottom two rows)', fontsize=14)
fig5.suptitle(f'{ref_subject} components: mean HRF and temporal-matched component/correlation per subject '
              f'(first {n_fig5_components} components)', fontsize=16)
plt.tight_layout()
plt.show()
