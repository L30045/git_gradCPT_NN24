...
filename_list = [os.path.basename(x) for x in glob.glob(os.path.join(eegpath,"*.csv"))]
...
for fname in filename_list:
    ...
    if "cpt" in fname.lower():
        ...
        continue
    # define savepath
    preproc_fname = os.path.join(eegpath,fname.split('.')[0]+'_preproc_eeg.fif')
    ica_fname = os.path.join(eegpath,fname.split('.')[0]+'_ica.fif')
    ...
    if os.path.exists(preproc_fname):
        ...
        # load EEG raw
        eeg_csv = pd.read_csv(os.path.join(eegpath,fname), skiprows=12, index_col=False)
...
files = [f for f in os.listdir(cptpath) 
            if os.path.isfile(os.path.join(cptpath, f)) 
            and f.endswith('.mat')]
# get EEG path
filename_list = [os.path.basename(x) for x in glob.glob(os.path.join(eegpath,"*.csv"))]
...
for fname in filename_list:
    ...
    data_cpt = sp.io.loadmat(os.path.join(cptpath,f_cpt))
    ...
    # save dataframe
    save_filename = os.path.join(cptpath, f'{fname.split('.')[0]}_events.tsv')
...
for key_name in subj_EEG_dict.keys():
    ...
    event_file = glob.glob(os.path.join(cptpath, f'*run-0{run_id}_events.tsv'))[0]
...
spectrum = EEG.compute_psd(fmin=0.5, fmax=100, n_fft=2048)
# Get PSD values and frequencies
psd, freqs = spectrum.get_data(return_freqs=True)
spectrum.plot()

#%% load fNIRS
nirspath = os.path.join(filepath, 'nirs')
nirs_fname = "ninjaNIRS2024_2026-04-13-13-52-36.bin"
