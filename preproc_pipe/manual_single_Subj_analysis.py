...
results_root = os.path.join("/projectnb/nphfnirs/s/datasets/gradCPT_NN24_pilot/sourcedata/raw/sub-Easycap/eeg/preprocessed/single_subject_results")
analysis_dirs = {
...
gz_path = os.path.join("/projectnb/nphfnirs/s/datasets/gradCPT_NN24_pilot/sourcedata/raw/sub-Easycap/eeg/preprocessed",
                       f"sub-{subj_id}_preprocessed_dict.pkl.gz")
...
key_name = next(iter(subj_EEG_dict.keys()))
...
loc_subj_epoch_dict = dict()
loc_subj_vtc_dict = dict()
loc_subj_react_dict = dict()
# always regenerate event files to ensure response_code matches BIDS nirs convention
gen_EEG_event_tsv(key_name.split('-')[-1],
                 data_path="/projectnb/nphfnirs/s/datasets/gradCPT_NN24_pilot/sourcedata/raw/sub-Easycap/eeg",
                 savepath="/projectnb/nphfnirs/s/datasets/gradCPT_NN24_pilot/sourcedata/raw/sub-Easycap/eeg",
                 gradcpt_path="/projectnb/nphfnirs/s/datasets/gradCPT_NN24_pilot/sourcedata/raw/sub-Easycap/gradCPT")
# for each run
interp_rt=False
for run_name in subj_EEG_dict[key_name]:
    if 'gradcpt' not in run_name:
        continue
    run_id = int(run_name.split('cpt')[-1])
    loc_subj_epoch_dict[f"run{run_id:02d}"] = dict()
    loc_subj_vtc_dict[f"run{run_id:02d}"] = dict()
    loc_subj_react_dict[f"run{run_id:02d}"] = dict()
    EEG = subj_EEG_dict[key_name][run_name]
    # load corresponding event file
    event_file = os.path.join("/projectnb/nphfnirs/s/datasets/gradCPT_NN24_pilot/sourcedata/raw/sub-Easycap/eeg",
    ...
    for select_event in event_labels_lookup.keys():
        if np.any(events[:,-1]==event_labels_lookup[select_event]):
            ...
            ev_react = []                                                  
        # save epochs
        loc_subj_epoch_dict[f"run{run_id:02d}"][select_event] = epochs
        loc_subj_vtc_dict[f"run{run_id:02d}"][select_event] = ev_vtc
        loc_subj_react_dict[f"run{run_id:02d}"][select_event] = ev_react
single_subj_epoch_dict=loc_subj_epoch_dict
single_subj_vtc_dict=loc_subj_vtc_dict
single_subj_react_dict=loc_subj_react_dict
